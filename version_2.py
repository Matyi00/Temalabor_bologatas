...
def array_has_signal(array):
    ...
    max_before_min = checked_array.index(max(checked_array)) < checked_array.index(min(checked_array))

    return (max_difference > 1) and max_greater_than_zero and min_less_than_zero and max_in_range and min_in_range and max_before_min
...
if __name__ == "__main__":
    ...
    vertical_array_landmarks = []
    vertical_array_procrustes = []
        ...
        while cap.isOpened():
            try:
                ...
                for idx, l in enumerate(landmarks):
                    ...
                    # img_1 = cv2.putText(img_1, str(idx), (int(l[0]), int(l[1])), font, fontScale, color, thickness, cv2.LINE_AA)
                #
                cv2.line(img_1, (int(landmarks[10][0]), int(landmarks[10][1])),
                         (int(landmarks[152][0]), int(landmarks[152][1])), (0, 0, 0), 2)
                cv2.line(img_1, (int(landmarks[446][0]), int(landmarks[446][1])),
                         (int(landmarks[226][0]), int(landmarks[226][1])), (0, 0, 0), 2)
                ...
                # ax1.plot(range(len(vertical_array_landmarks)), scipy.ndimage.gaussian_filter1d(vertical_array_landmarks, 4.5), label="vertical")
    ...
    #    if cv2.waitKey(1) & 0xFF == ord('q'):
    #        break
    # ax1.plot(range(len(vertical_array_landmarks)), scipy.ndimage.gaussian_filter1d(vertical_array_landmarks, 4.5),
    #          label="vertical")
    ax1.plot(range(len(vertical_array_landmarks)), vertical_array_landmarks,
             label="relative")
    ax2.plot(range(len(vertical_array_procrustes)), vertical_array_procrustes, label="nods")
    ax1.legend()
    ax2.legend()
    plt.show()