...
def array_has_signal(array):
    global single_sign_counter
    checked_array_size = 20
    checked_array = array[-checked_array_size:]
    min_value = min(checked_array)
    max_value = max(checked_array)
    max_difference = max_value - min_value
    max_greater_than_zero = max_value > 0
    min_less_than_zero = min_value < 0
    max_in_range = abs(max_value) > (0.75 * abs(min_value))
    min_in_range = abs(min_value) > (0.75 * abs(max_value))

    if (max_difference > 1) and max_greater_than_zero and min_less_than_zero and max_in_range and min_in_range and single_sign_counter < 0:
        single_sign_counter = checked_array_size
        return True
    single_sign_counter -= 1
    return False
...
if __name__ == "__main__":
    ...
    # video_path = "C:\\Users\\tassm\\OneDrive\\Documents\\data\\BAUM1a\\s001\\S001_003.mp4"
